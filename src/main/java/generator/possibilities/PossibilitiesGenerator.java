...
@Value
@Builder
public class PossibilitiesGenerator implements Callable<PossibilitiesFactory.StampedAnchor> {
    ...
    @Override
    @SneakyThrows
    public PossibilitiesFactory.StampedAnchor call() {
        return new PossibilitiesFactory.StampedAnchor(
                coordinate.row(),
                coordinate.col(),
...
//
//    private Function<Coordinate, PossibilitiesFactory.StampedAnchor> stamp(Board board) {
//        return coordinate -> new PossibilitiesFactory.StampedAnchor(
//                coordinate.row(),
//                coordinate.col(),
//                calculatePossibilities(coordinate, board)
//        );
//    }
    ...
    private Set<Character> possibilitiesDown(Coordinate coordinate, Board board) {
        ...
        Set<PossibilitiesFactory.Traversal> traversals = initialTraversals(dictionary.getDictionary().getRoot());
    ...
    record Traversal(String path, TrieNode node, Character start) {
        ...
        static Set<PossibilitiesFactory.Traversal> findNeighbours(Set<PossibilitiesFactory.Traversal> traversals, Field field) {
            if (field.isEmpty()) {
                return traversals;
            }
            ...
            return traversals.stream()
                    ...
                    .map(traversal -> new PossibilitiesFactory.Traversal(
        ...
        static Set<PossibilitiesFactory.Traversal> initialTraversals(TrieNode root) {
            return wholeAlphabet().stream()
                    .filter(root::hasNeighbour)
                    .map(root::getNeighbour)
                    .flatMap(Optional::stream)
                    .map(node -> new PossibilitiesFactory.Traversal(
        ...
        static Set<Character> toCharacterSet(Set<PossibilitiesFactory.Traversal> traversals) {
            return traversals.stream()
                    .map(PossibilitiesFactory.Traversal::start)
                    .collect(toSet());
        }
    };
}
